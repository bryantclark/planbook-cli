...
def test_check_maps_not_logged_in_to_not_authenticated():
    with pytest.raises(NotAuthenticated):
        client()._check(response(200, '{"notLoggedIn":"true"}'), "https://api.planbook.com/getClasses2")


def test_check_maps_error_body_to_api_error_message():
    with pytest.raises(ApiError, match="bad field"):
        client()._check(response(200, '{"error":"true","msg":"bad field"}'), "https://api.planbook.com/updateLesson")


def test_check_maps_non_json_to_schema_drift():
    with pytest.raises(SchemaDrift, match="non-JSON"):
        client()._check(response(200, "<html>nope</html>"), "https://api.planbook.com/getClasses2")


def test_check_maps_waf_405_to_schema_drift():
    with pytest.raises(SchemaDrift, match="AWS WAF"):
        client()._check(response(405, "AWSWAF human verification"), "https://api.planbook.com/getClasses2")


def test_check_returns_normal_dict_unchanged():
    body = client()._check(response(200, '{"classes":[]}'), "https://api.planbook.com/getClasses2")
    assert body == {"classes": []}


def test_require_names_missing_keys():
    with pytest.raises(SchemaDrift) as exc:
        client().require({"classes": []}, "classes", "currentYearId", "lessonBanks", where="getClasses2")
