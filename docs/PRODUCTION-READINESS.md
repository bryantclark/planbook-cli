...
| Live validation | One account, one teacher, by hand. Nothing automated reads the real API, so a field Planbook renames is found when a command breaks, not before. What hand-checking has found is in `docs/API-NOTES.md` |
